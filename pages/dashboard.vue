<template>
  <div class="container mx-auto p-6">
    ...
    <div class="flex justify-between items-center mt-4">
      <button @click="editPost(post.id)" class="flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-all duration-300 ease-in-out shadow-md text-sm">
      ...
      <button @click="deletePost(post.id)" class="px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700 transition-all duration-300 ease-in-out shadow-md text-sm">
        Delete
      </button>
    </div>
...
export default {
  ...
  methods: {
    ...
    editPost(postId) {
      // Navigate to the edit page, passing the post ID in the route params
      this.$router.push({ name: 'edit-post', params: { id: postId } });
    },
    deletePost(postId) {
      // Perform delete action here (API call and state update)
